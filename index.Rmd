...
in the resulting subsamples. The ***networktree*** package implements two network tree algorithms:
...
```{r networktree, echo = FALSE, fig.width=14, fig.height=8, dev="svg"}
...
tree_complex <- networktree(nodevars=dass[,dNames], splitvars=dass[,c("age", "race"),drop=FALSE])
plot(tree_complex,transform="pcor",
     labels=dNames, vsize=15, label.norm="OOOOO",
     partyargs=list(ep_args = list(justmin = 8), tnex = 2.5))
```
